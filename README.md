...
## How the OIDC authentication works

```text
1. GitHub Actions starts on a job targeting environment: DEV/UAT/PROD
2. GitHub issues an OIDC token for that job, encoding the environment claim
3. azure/login sends the token to Azure
4. Azure checks the federated credential matching:
     repo = <GITHUB_ORG>/<GITHUB_REPO>
     environment = DEV | UAT | PROD   (must match the credential's subject)
5. Azure authenticates the application (no secret exchanged)
6. AcrPush permission is checked
7. Docker image is pushed to ACR
```

Each environment's federated credential only matches jobs that declare
that exact `environment:`. A workflow targeting `DEV` can't authenticate
using the `PROD` credential, since the OIDC token's `sub` claim is
different.

No Azure client secret or password is ever stored in GitHub, only the
`AZURE_CLIENT_ID`, `AZURE_TENANT_ID`, and `AZURE_SUBSCRIPTION_ID` values,
which are identifiers rather than credentials.
